...
统一处理家电、数码国补 Excel 数据，支持已上传数据、收款单、审核明细和回款明细。
...
| 收款单统计 | 文件名包含 `收款单统计` 的 `.xls` | `收款单统计.xlsx` |
| 审核明细 | 家电、数码合并在同一个 `销售用券情况统计*.xls` 内（按行区分：每行仅 2026家电国补或2026数码国补其中一列有值） | `审核明细.xlsx` |
| 回款明细（家电+数码） | 文件名包含 `补贴明细` | `回款明细.xlsx` |

回款明细支持 `.xlsx`、`.xls`、`.xlsm`、`.csv` 和 `.tsv`。程序会根据文件名或
表头自动区分家电和数码，并按照 `config/payment_brands.yaml` 进行品类映射和品牌
识别。`.xlsx` 来源只读打开；其他格式产生的临时工作副本会在处理结束后删除。
